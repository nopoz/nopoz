...
Red-teaming AI agent that runs a full pentest campaign (recon, exploitation, post-exploitation, lateral movement, persistence, and a ranked report), with the model reasoning but never touching the network. Claude returns schema-validated JSON while a LangGraph state machine owns scope and runs every tool behind a fail-closed scope guard that denies by default. Human-approval gates are real checkpointed graph interrupts the model cannot skip, and the brain authors exploits for operator review before they run. The operator drives the whole campaign from a live web console that streams tool activity in real time and is where every approval gate is cleared. Every phase is MITRE ATT&CK-aligned with per-tactic KPIs and audit trails in LangSmith, and any live run replays deterministically. Source is private; demo available on request.
